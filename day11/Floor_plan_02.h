...
private:
  ...
  Seat m_get_neighbors_ray(size_t row, size_t col, int d1, int d2) const;
...
Seat Floor_plan::m_get_neighbors_ray(size_t row, size_t col, int dr, int dc) const
{
  int n{1};
  Seat seat;
  do
  {
    seat = get_safely(row + (n * dr), col + (n * dc));
    n += 1;
  } while (seat == Floor);
  return seat;
}
...
fparr Floor_plan::get_neighbors(size_t row, size_t col) const
{
  fparr ret;
  ret.reserve(8);
  m_pushback_neigh(ret, m_get_neighbors_ray(row, col, -1, -1));
  m_pushback_neigh(ret, m_get_neighbors_ray(row, col, -1, 0));
  m_pushback_neigh(ret, m_get_neighbors_ray(row, col, -1, 1));
  m_pushback_neigh(ret, m_get_neighbors_ray(row, col, 0, -1));
  m_pushback_neigh(ret, m_get_neighbors_ray(row, col, 0, 1));
  m_pushback_neigh(ret, m_get_neighbors_ray(row, col, 1, -1));
  m_pushback_neigh(ret, m_get_neighbors_ray(row, col, 1, 0));
  m_pushback_neigh(ret, m_get_neighbors_ray(row, col, 1, 1));
...
void Floor_plan::apply_rules()
{
  ...
  for (size_t r{0}; r != m_rows; ++r)
  {
    for (size_t c{0}; c != m_cols; ++c)
    {
      ...
      {
        neigh = get_neighbors(r, c);
        if (std::count(neigh.begin(), neigh.end(), Occ) >= 5)
          tmp.push_back(Empty);
        else
          tmp.push_back(seat);
      }
