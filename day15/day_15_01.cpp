...
private:
  ...
  std::unordered_map<size_t, std::vector<size_t>> m_turns{};
...
void Game::add_number(size_t number)
{
  ...
  if (pos == m_turns.end())
    // m_turns[number] = std::vector<size_t>{m_current_turn};
    m_turns.emplace(number, std::vector<size_t>{m_current_turn});
  else
    m_turns[number].push_back(m_current_turn);
...
void Game::take_turn()
{
  ...
  if ((pos == m_turns.end()) || (pos->second.size() == 1))
  {
    m_last_number = 0;
...
namespace
{
  ...
  {
    std::cout << "Test case 1 target: 436" << '\n';
    return run_game(std::vector<size_t>{0, 3, 6}, 2020);
  }
  size_t provided_test_02()
  {
    std::cout << "Test case 2 target: 1" << '\n';
    return run_game(std::vector<size_t>{1, 3, 2}, 2020);
  }
  size_t provided_test_03()
  {
    std::cout << "Test case 3 target: 10" << '\n';
    return run_game(std::vector<size_t>{2, 1, 3}, 2020);
  }
  size_t provided_test_04()
  {
    std::cout << "Test case 4 target: 27" << '\n';
    return run_game(std::vector<size_t>{1, 2, 3}, 2020);
  }
  size_t provided_test_05()
  {
    std::cout << "Test case 5 target: 78" << '\n';
    return run_game(std::vector<size_t>{2, 3, 1}, 2020);
  }
  size_t provided_test_06()
  {
    std::cout << "Test case 6 target: 438" << '\n';
    return run_game(std::vector<size_t>{3, 2, 1}, 2020);
  }
  size_t provided_test_07()
  {
    std::cout << "Test case 7 target: 1836" << '\n';
    return run_game(std::vector<size_t>{3, 1, 2}, 2020);
  }
} // namespace
...
int main()
{
...

  {
    std::cout << "Result of sequence 0,6,1,7,2,19,20: "
              << run_game(std::vector<size_t>{0, 6, 1, 7, 2, 19, 20}, 2020) << '\n';
  }
  return EXIT_SUCCESS;
}